...
def clip_classifier(classnames, template, clip_model, tokenizer, device):
    with torch.no_grad():
        ...
        text_feature = torch.stack(clip_weights, dim=1).to(device)
    return text_feature
...
def get_clip_logits(images, clip_model, text_feature):
    with torch.no_grad():
        ...
        images = images.to(text_feature.device)
...
def get_res_logits(images, clip_model, text_feature, weights):
    with torch.no_grad():
        weights = weights.to(device=text_feature.device, dtype=text_feature.dtype)
        if isinstance(images, list):
            images = torch.cat(images, dim=0)
        images = images.to(text_feature.device)
        image_features = clip_model.encode_image(images)
...
def build_test_data_loader(dataset_name, root_path, preprocess, batch_size=512):
    if dataset_name == 'I':
        dataset = build_dataset("imagenet", root_path)
        test_loader = build_data_loader(data_source=dataset.test, batch_size=batch_size, is_train=False, tfm=preprocess, shuffle=True)
